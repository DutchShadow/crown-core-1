...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorMessage, CMasternodeBroadcast &mnb, bool fOffline) {
    ...
    bool fSignOver = true;
    return Create(txin, CService(LookupNumeric(strService.c_str())), keyCollateralAddress, pubKeyCollateralAddress, keyMasternodeNew, pubKeyMasternodeNew, fSignOver, strErrorMessage, mnb);
}
...
bool CMasternodePing::Sign(const CKey& keyMasternode, const CPubKey& pubKeyMasternode)
{
    ...
    if(!legacySigner.SignMessage(hash.GetHex(), errorMessage, vchSigPrevBlocks, keyMasternode)) {
        LogPrint(BCLog::NET, "CMasternodePing::Sign() - Error signing previous blocks: %s\n", errorMessage);
        return false;
    }
