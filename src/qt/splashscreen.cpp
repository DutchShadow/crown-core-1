    ...
    QWidget(0, f), curAlignment(0), m_node(node)
{
    ...
    QString copyrightTextCROWN  = QChar(0xA9) + QString(" 2018 ") + QString(tr("The Crown developers"));
    QString titleAddText = networkStyle->getTitleAddText();

    // create a bitmap according to device pixelratio
    QSize splashSize(500*devicePixelRatio,567*devicePixelRatio);
    pixmap = QPixmap(splashSize);
    QPainter pixPaint(&pixmap);
    pixPaint.setPen(QColor(100,100,100));

    // draw a slightly radial gradient
    QRect rectIcon(QPoint(0,0), QSize(500,567));
    const QSize requiredSize(500,567);
    ...
    pixPaint.drawText(paddingLeft, paddingTop + titleCopyrightVSpace + 103, copyrightTextDash);
    pixPaint.drawText(paddingLeft, paddingTop + titleCopyrightVSpace + 115, copyrightTextCROWN);
